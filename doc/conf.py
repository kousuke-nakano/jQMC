"""Sphinx configuration of jQMC documentation."""

# -*- coding: utf-8 -*-
#
# jQMC documentation build configuration file, created by
...
# Make jqmc and jqmc_workflow importable for autodoc
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent))


# ---------------------------------------------------------------------------
# Auto-generate doc/examples.md from examples/*/README.md
# ---------------------------------------------------------------------------
def _generate_examples_page():
    ...
    * Runs at import time so the file is ready before Sphinx reads sources.
    ...
    for readme in sorted(examples_dir.glob("jqmc-*/README.md")):
        example_name = readme.parent.name
        rel_dir = f"../examples/{example_name}"
        ...
        def _fix_image(m, _dir=rel_dir):
            alt, path = m.group(1), m.group(2)
            if path.startswith(("http://", "https://")):
                return m.group(0)
            return f"![{alt}]({_dir}/{path})"

        content = _re.sub(r"!\[([^\]]*)\]\(([^)]+)\)", _fix_image, content)

        # Remove duplicate footnote definitions
        def _dedup_footnote(m):
            label = m.group(1)
            if label in seen_footnotes:
                return ""  # drop duplicate
            seen_footnotes.add(label)
            return m.group(0)
    ...
    # Only write when content actually changed (avoids unnecessary rebuilds)
    if out_path.exists() and out_path.read_text("utf-8") == output:
        return
...
# of the sideba.
# html_logo = '_static/logo_yoko2.jpg'
...
latex_documents = [
    ("documentation", "jQMC.tex", "jQMC manual", "Kosuke Nakano", "manual"),
]
...
def run_scripts(app):
    """Run scripts to generate images."""
    script_path = os.path.join(app.srcdir, "notes/paulinet_rbf_plot.py")
